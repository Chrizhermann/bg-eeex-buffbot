...
### Testing
- Automated coverage includes real WeiDU 249 English and Simplified Chinese synthetic installs across EEex v0.11 and v1, byte-exact selected catalogs, innate-only active-TLK changes, inactive/root TLK stability, current and released-v1.7 lifecycle switching, pre-existing catalog ownership restoration, the map-backed candidate migration, raw-deploy preservation, and installation from the packaged Chinese archive. The final complete-suite total will be recorded after the live-validation task's final run.
- Corrected live Simplified Chinese startup, CJK glyph/layout, and Copy Copy acceptance are still pending and are not claimed here, including real runtime Project Image safety, non-ASCII export filenames, and save/reload. BG1EE, EET, alternate resolutions and fonts, the Project Infinity frontend, and WeiDU versions other than 249 also remain unvalidated.
