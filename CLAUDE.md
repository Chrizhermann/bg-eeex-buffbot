...
- **Quick Cast (cheat mode)**: `BfBot.Exec.Start(queue, qcMode)` accepts optional qcMode (0=off, 1=long only, 2=all). User spell priority always respected — no reordering by duration category ("list order is king"). BFBTCH.SPL (Improved Alacrity + casting speed reduction) applied before first entry via `ReallyForceSpellRES`, covers the entire queue. BFBTCR.SPL (opcode 321 remove by resource) applied at `_Complete()` cleanup. Cleanup on Stop() also removes lingering BFBTCH.
...
- **Cast order**: User-set `pri` is always respected, regardless of Quick Cast mode. No reordering by duration category. For any qcMode >= 1, IA (BFBTCH) is applied before the first entry and removed at completion — the entire queue casts under Improved Alacrity. `cheatBoundary = #entries` for the whole queue.
