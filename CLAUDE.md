...
- **Config schema v9**: `{v=9, ap, presets=[{name,cat,qc,spells={[resref]={on,tgt,pri,rep,tgtUnlock,lock,var}}}], opts={skip}, ovr={[resref]=1|-1}, summons={[identity]={presets={[n]={qc,spells={[resref]={on,tgt,pri,rep,var}}}}}}}`. `var` is optional. `rep` must be a finite integer from `1` through `BfBot.MAX_SPELL_REPEATS` (currently 5); missing or invalid values become `1`. `tgt` = `"s"` / `"p"` / character name / ordered table of names. Legacy slot strings (`"1"`-`"6"`) lazily convert to names in `_Refresh`. Only the protagonist's `summons` subtree is read/written. Older saves migrate lazily; v8→v9 initializes `rep` in both party and summon presets.
...
- **Project Image lock policy**: a live PI locks its owner. Skip all owner entries while its PI exists, retain only one Project Image attempt even when configured higher, and drop (never reorder) owner entries after a Project Image cast. Copied BuffBot F12 innates on clones are not supported in v1.6.0-alpha; use the Summons view / Cast All paths.
