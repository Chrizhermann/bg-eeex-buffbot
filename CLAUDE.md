...
- **Target picker**: BUFFBOT_TARGETS sub-menu with ordered priority list. Self/All Party quick-set buttons, 6 player rows in fixed portrait order with priority numbers (`[N] Name`), Move Up/Down for reordering, Clear/Done buttons. **Lock gating**: self-only spells locked to "Self", AoE locked to "Party" (based on `isAoE`/`isSelfOnly` scan flags). "Unlock Targeting" button overrides lock via `tgtUnlock=1` on spell config (for modded spells). **Name-based storage**: targets stored as character names (e.g., `{"Branwen","Ajantis"}`) instead of slot numbers. Dual-format: old slot strings ("1"-"6") accepted and lazily converted to names in `_Refresh()`. Resolution via `_ResolveNameToSlot(name)` at cast time. Functions: `PickerToggle`, `PickerSelf`, `PickerAllParty`, `PickerMoveUp/Down`, `PickerSelect`, `PickerClear`, `PickerDone`, `PickerUnlock`.
