...
**Known remaining edge case**: Spell Revisions Barkskin — SR delivers AC bonus via sub-spell (opcode 146), leaving the main SPL with only infrastructure opcodes. Classified as AMB (ambiguous, leaning buff) instead of definitive BUFF. Manual override via config UI will handle this (GitHub issue #1). *Duration* for this shape of spell is now read correctly — `GetDuration` recurses through op=146 sub-spells (#33) — but the classification score itself still doesn't recurse, so the AMB call stands.
