...
- `tools/deploy.sh` — dev deploy (preserves a coherent installer-owned localization pair; otherwise patches the English innate-name TLK fallback)
...
- **Localization ownership boundary**: BuffBot-owned UI, Options, defaults, and player feedback load as UTF-8 from `override/bfbot_l10n.tra`, with checked-in per-key English fallback. Never route those runtime catalog IDs through `Infinity_FetchString`; a native fetch can hard-crash before the selected TLK is ready even after the main-file-loaded listener. Only the eight generated F12 SPL names remain TLK-backed via `bfbot_strrefs.txt`; fetching engine-owned spell/item names at proven runtime boundaries remains valid. During same-process WeiDU language switches, the selected catalog directory comes from reserved TRA metadata `@113`, not `%LANGUAGE%`, because WeiDU 249's uninstall phase can restore the previous directory value.
- **Up to 8 presets** via `BfBot.MAX_PRESETS`.
- **New-companion preset inheritance**: only a config-less party companion inherits the protagonist's exact preset indices, names, and categories. The copy is structure-only and deep, Quick Cast is Off, every entry from the recruit's own scan starts disabled, and an existing or returning companion config is never reconciled or overwritten.
