...
- `BuildQueueFromPreset(N)`: after the party loop, if `GetPref("SummonsJoinCast") == 1`, for each `GetAlliedSummons()` entry passing the MP rule (Task 13; SP short-circuits true) with a non-empty enabled preset-N table, append `BuildQueueForSummon(...)` entries. NOTE (probe): if the owner casts PI in the same run, the owner is puppet-LOCKED until the clone expires — order PI last per owner or let the watchdog treat locked owners as busy.
